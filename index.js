...
while (input !== 'q') {
  ...
  // Get user input
  input = conif.getConsoleInput('Input a number between 1-3999 to generate its roman numeral equivilant. (q to quit):   ');

  // Check user input
  if (input >= 1 && input <= 3999) {
    console.log(input, 'in roman numerals is:', romanNumeralGenerator.generate(input));
  } else if (input === 'q') {
    console.log('Goodbye');
  } else {
